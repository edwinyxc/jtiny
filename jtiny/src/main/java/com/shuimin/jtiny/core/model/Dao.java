...
import com.shuimin.jtiny.db.DbOper;
import com.shuimin.jtiny.db.JdbcTmpl;
...
public abstract class Dao<M extends Model> {
	...
	private Connection _getConn() {
		try {
			return Y.ctx().connectionPool().getConnection();
		} catch (SQLException ex) {
			Y.logger().info("failed to get connection");
			Y.debug(ex);
		}
		return null;
	}
